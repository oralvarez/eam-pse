...
class Cliente(models.Model):
    ...
    logo = models.ImageField(upload_to=content_file_name, max_length=500)
...
class Dependencia(models.Model):
    ...
    dependencia_padre = models.ForeignKey('self', null=True, blank=True)

    def __unicode__(self):
        return 'CLIENTE: %s - IDENTIFICACION: %s - TIPO: %s - NOMBRE: %s' % (
            self.cliente, self.identificacion, self.tipo, self.nombre)
...
class Producto(models.Model):
    ...
    # require OneToManyFields(Dependencia)
    ...
    gestor_asignado = models.ForeignKey(User, blank=True, null=True, related_name='gestor_asignado')
    asignador = models.ForeignKey(User, blank=True, null=True, related_name='asignador')
...
class Estado_Producto(models.Model):
    ...
    def __unicode__(self):
        return '%s - %s - %s - %s - %s - %s - %s' % (
            self.producto, self.usuario, self.estado_anterior, self.accion, self.estado_actual, self.fecha,
            self.descripcion)
...
class Detalle_Servicio(models.Model):
    ...
    porcentaje = models.DecimalField(max_digits=4, decimal_places=1, default=100)
    funcionario_autorizado = models.TextField(max_length=150)

    def __unicode__(self):
        return '%s - %s - %s - %s' % (self.producto, self.dependencia, self.porcentaje, self.funcionario_autorizado)
...
def enviar_notificacion_actualizacion_producto(sender, instance, created, *args, **kwargs):
    from_message = "no-reply@eamcys.com"
    ...
    recipients = []
    for user in User.objects.filter(groups__name='Asignadores'):
    ...
    if created:
        instance.numero_consecutivo = Producto.objects.all().aggregate(Max('numero_consecutivo')).get(
            'numero_consecutivo__max') + 1
        instance.consecutivo = "EAM-" + str(instance.numero_consecutivo)
        ...
        message = render_to_string('tema3/correo.html',
                                   {'usuario': nombre_usuario, 'consecutivo': instance.consecutivo})
    else:
